...
// ****************************************************************************
    ...
    // Resolver Fbk - Cosine @ b4
    // ********************************
    AdcbRegs.ADCSOC0CTL.bit.CHSEL     = 4;   // SOC0 will convert pin C15
    AdcbRegs.ADCSOC0CTL.bit.ACQPS     = 30;   // sample window in SYSCLK cycles
    AdcbRegs.ADCSOC0CTL.bit.TRIGSEL   = 15;   // trigger on ePWM6 SOCA/C
    // Configure PPB to eliminate subtraction related calculation
    AdcbRegs.ADCPPB1CONFIG.bit.CONFIG = 0;    // PPB is associated with SOC0
    AdcbRegs.ADCPPB1OFFCAL.bit.OFFCAL = 0;    // Write zero to this for now till offset ISR is run

    // Resolver Fbk - sine @ a4
    // ********************************
    AdcaRegs.ADCSOC0CTL.bit.CHSEL     = 4;    // SOC0 will convert pin D1
    AdcaRegs.ADCSOC0CTL.bit.ACQPS     = 30;   // sample window in SYSCLK cycles
    ...
    // Phase Voltage sense, A3 SOC2 ADCA, Vfb-V(B) C3
    // ********************************
    AdccRegs.ADCSOC2CTL.bit.CHSEL     = 3;    // SOC2 will convert pin A3,
    AdccRegs.ADCSOC2CTL.bit.ACQPS     = 30;   // sample window in SYSCLK cycles
    AdccRegs.ADCSOC2CTL.bit.TRIGSEL   = 5;    // trigger on ePWM1 SOCA/C
    // Configure PPB to eliminate subtraction related calculation
    AdccRegs.ADCPPB3CONFIG.bit.CONFIG = 1;    // PPB is associated with SOC1
    ...
    //C3, Vfb-U(A) adcin14_porta
    // ********************************
    AdcaRegs.ADCSOC2CTL.bit.CHSEL     = 14;    // SOC2 will convert pin C3
