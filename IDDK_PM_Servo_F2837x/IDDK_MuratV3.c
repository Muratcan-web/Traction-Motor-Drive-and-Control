...
_iq offset_shntV,        // offset in shunt current V fbk channel @ 0A
    offset_shntW,        // offset in shunt current W fbk channel @ 0A
    offset_shntU,        // offset in shunt current U fbk channel @ 0A
    offset_Rsin,         // offset in resolver sine fbk channel @ no excitation
...
/*    current_sensor[LEM_CURRENT_SENSE-1].Bs   = (float)IFB_LEMW_PPB* ADC_PU_PPB_SCALE_FACTOR * LEM_TO_SHUNT;
    ...
    current_sensor[LEM_CURRENT_SENSE-1].Cs   = (float)IFB_LEMW_PPB* ADC_PU_PPB_SCALE_FACTOR * LEM_TO_SHUNT;
    current_sensor[LEM_CURRENT_SENSE-1].Bs   = -current_sensor[LEM_CURRENT_SENSE-1].As
                                               -current_sensor[LEM_CURRENT_SENSE-1].Cs;
...
// symbols are created by the linker. Refer to the linker files.
    ...
    InitSysCtrl();
    //MY folowing three lines are added
    EALLOW;
...
// ****************************************************************************
    ...
    TrigRegs.SYNCSELECT.bit.EPWM4SYNCIN = 0;     //EPwm1SyncOut
    ...
    TrigRegs.SYNCSELECT.bit.EPWM10SYNCIN = 0;  //EPwm1Sync Out
...
*/
    // Resolver Fbk - Cosine @ C1  //MY cos: C2
    // ********************************
    ...
    // Resolver Fbk - sine @ D1  //MY: sin D1
    // ********************************
    ...
    // LEM motor current LEM-V @ at A2 //MY: A3
    // ********************************
    AdcaRegs.ADCSOC1CTL.bit.CHSEL     = 3;    // SOC1 will convert pin A2
    ...
    // LEM motor current LEM-W @ at B2 //MY: B2
    // ********************************
    ...
    // Phase Voltage sense, A3 SOC2 ADCA, Vfb-V (B)
    // ********************************
...
// ****************************************************************************
    ...
    // Init QEP parameters
    qep1.LineEncoder = 360; //MY 2500; // these are the number of slots in the QEP encoder
...
// ****************************************************************************
    ...
    AdcaRegs.ADCPPB1OFFREF = (offset_shntW*4096.0);      // setting shunt Iu offset
    AdcbRegs.ADCPPB1OFFREF = (offset_shntV*4096.0);      // setting shunt Iv offset

    AdccRegs.ADCPPB2OFFREF = (offset_Rcos*4096.0);      // setting resolver cos in offset
    ...
    AdcdRegs.ADCPPB2OFFREF = (offset_Rcos*4096.0);      // setting resolver cos in offset
    ...
    AdccRegs.ADCPPB2OFFREF = (offset_shntU*4096.0);      // setting shunt Iw offset
...
// Tune the Digital Filter Module of CMPSS
...
Uint16  clkPrescale = 20,
        ...
        SHUNT_curHi = 2048 + SHUNT(8),  //1647,
        SHUNT_curLo = 2048 - SHUNT(8);  //1647;

void  CMP_DIG_FILTER(volatile struct CMPSS_REGS *v, Uint16 curHi, Uint16 curLo) {
    // comparator references
    v->DACHVALS.bit.DACVAL = curHi;
    v->DACLVALS.bit.DACVAL = curLo;
...
//--------------------------------------------------------
{
    ...
    // *******************************************************
    EALLOW;
      CMP_DIG_FILTER(&Cmpss1Regs, LEM_curHi, LEM_curLo);      // LEM - V
      CMP_DIG_FILTER(&Cmpss3Regs, LEM_curHi, LEM_curLo);      // LEM - W
      CMP_DIG_FILTER(&Cmpss2Regs, SHUNT_curHi, SHUNT_curLo);  // SHUNT - V
      CMP_DIG_FILTER(&Cmpss6Regs, SHUNT_curHi, SHUNT_curLo);  // SHUNT - U
    EDIS;
//MY comment out
    // Check for PWM trip due to over current

    if(EPwm1Regs.TZFLG.bit.OST == 0x1)
        TripFlagDMC = 0;      // Trip on DMC (halt and IPM fault trip ) // it was originally TripFlagDMC=1;
...
//  else        pi_id.Ref = IdRef;
    pi_id.Ref = ramper(IdRef, pi_id.Ref, _IQ(0.0001));
    pi_id.Fbk = park1.Ds;
    PI_MACRO(pi_id)
